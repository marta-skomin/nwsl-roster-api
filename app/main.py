...
def get_player_endpoint(name: str, db: Session = Depends(get_db)):
...
def add_player(player: Player, db: Session = Depends(get_db)):
...
def get_squad_stats(team_name: str, db: Session = Depends(get_db)):
    players = get_team_players(team_name, db)
    if not players:
        raise HTTPException(status_code=404, detail=f"Team '{team_name}' not found")
